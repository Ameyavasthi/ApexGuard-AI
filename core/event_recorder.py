...
class EventRecorder:
    ...
    def _record_video_thread(self, filepath: str, pre_event_frames: list):
        ...
        try:
            ...
            # Use native webm format (VP80) to guarantee browser playback support
            # since Windows cv2 often missing H264 codecs
            out = None
            ...
            # Phase 1: Write Pre-Event Cache Safely
            ...
            # Phase 2: Consume real-time incoming frames securely enforcing strict shape constraints
            while frames_written < target_post_frames:
                try:
                    frame = self._active_frame_queue.get(timeout=1.0)
                    if frame is not None:
                        # Resize frame to match writer dimensions if webcam resolution fluctuated
            ...
            if out is not None:
                # MANDATORY release call guarantees the WebM header wraps accurately closing the 0:00 bug
                out.release()
            with self._lock:
                self.is_recording = False
                self._active_frame_queue = None
